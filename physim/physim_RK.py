...
def tot_acc(systm, part, pos, vel):
    systm[part].acc =[0,0,0]
    B = mag_field(systm,pos)
    E = elec_field(systm, pos)
    v_x_B = cross(vel,B)
    GF = grav_field(systm,pos)

    for cord in range(3):
        systm[part].acc[cord] = (systm[part].charge/systm[part].mass)*(E[cord] + v_x_B[cord])+GF[cord]

    #return systm[part].acc
...
k0 =[[0 for i in range(3)] for j in range(num)]
k1 =[[0 for i in range(3)] for j in range(num)]
k2 =[[0 for i in range(3)] for j in range(num)]
k3 =[[0 for i in range(3)] for j in range(num)]

l0 =[[0 for i in range(3)] for j in range(num)]
l1 =[[0 for i in range(3)] for j in range(num)]
l2 =[[0 for i in range(3)] for j in range(num)]
l3 =[[0 for i in range(3)] for j in range(num)]
...
for  i in range(timesteps):
    ...
    for par0 in range(num):
        #print systm[par].acc
        tot_acc(systm,par0,systm[par0].pos,systm[par0].vel)
        for k0cord in range(3):
            k0[par0][k0cord] = timeres*systm[par0].vel[k0cord]
            l0[par0][k0cord] = timeres*systm[par0].acc[k0cord]

    for par1 in range(num):
        #print systm[par].acc
        temp_k1pos = [0,0,0]
        temp_l1vel = [0,0,0]
        #temp_fun1 =[0,0,0]
        for i in range(3):
            temp_k1pos[i] = systm[par1].pos[i]+(k0[par1][i]/2.0)
            temp_l1vel[i] = systm[par1].vel[i]+(l0[par1][i]/2.0)
        tot_acc(systm,par1,temp_k1pos,temp_l1vel)
        for k1cord in range(3):
            k1[par1][k1cord] = timeres*(systm[par1].vel[k1cord]+ (l0[par1][k1cord]/2.0))
            l1[par1][k1cord] = timeres*systm[par1].acc[k1cord]

    for par2 in range(num):
            #print systm[par].acc
        temp_k2pos = [0,0,0]
        temp_l2vel = [0,0,0]
        #temp_fun2 = [0,0,0]
        for j in range(3):
            temp_k2pos[j] = systm[par2].pos[j]+(k1[par2][j]/2.0)
            temp_l2vel[j] = systm[par2].vel[j]+(l1[par2][j]/2.0)
        tot_acc(systm,par2,temp_k2pos,temp_l2vel)
        for k2cord in range(3):
            k2[par2][k2cord] = timeres*systm[par2].vel[k2cord]
            l2[par2][k2cord] = timeres*systm[par2].acc[k2cord]

    for par3 in range(num):
        #print systm[par].acc
        temp_k3pos =[0,0,0]
        temp_l3vel = [0,0,0]
        #temp_fun3 = [0,0,0]
        for k in range(3):
            temp_k3pos[k] = systm[par1].pos[k]+k2[par3][k]
            temp_l3vel[k] = systm[par1].vel[k]+l2[par3][k]
        tot_acc(systm,par3,temp_k3pos,temp_l3vel)

        for k3cord in range(3):
            k3[par3][k3cord] = timeres*systm[par3].vel[k3cord]
            l3[par3][k3cord] = timeres*systm[par3].acc[k3cord]

    for par in range(num):
        for cord in range(3):
            systm[par].pos[cord] = systm[par].pos[cord] + (k0[par][cord] + 2.0*k1[par][cord]+ 2.0*k2[par][cord] + k3[par][cord])/6.0
            systm[par].vel[cord] = systm[par].vel[cord] + (l0[par][cord] + 2.0*l1[par][cord]+ 2.0*l2[par][cord] + l3[par][cord])/6.0

            #systm[par].acc[cord] = 0
